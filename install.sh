...
spinner(){
  local pid=$1 msg=${2:-Working}; local spin='|/-\\'; local i=0
  while kill -0 "$pid" 2>/dev/null; do i=$(( (i+1) %4 )); printf "\r%s %s" "${spin:$i:1}" "$msg"; sleep .1; done
  printf "\r✓ %s\n" "$msg"
}
...
install_systemd_units(){
  ...
  systemctl --user enable forge-heartbeat.timer forge-observer.timer || true
}

menu(){
  banner
  PS3=$'\nChoose ForgeOS action: '
  select opt in "Full bootstrap" "Base packages" "Tauri 2/Rust desktop deps" "Terminal configs" "Sandbox tooling" "Systemd user timers" "Show post-wipe commands" "Quit"; do
    case "$REPLY" in
      1) apt_update; install_base; install_bun; install_tauri; install_bubbletea_cli; setup_dirs; write_configs; install_systemd_units; log "${GREEN}Full bootstrap complete.${NC}";;
